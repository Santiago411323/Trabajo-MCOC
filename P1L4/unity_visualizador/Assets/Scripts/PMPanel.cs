...
public class PMPanel : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        float px = Screen.width > 840f
            ? (AreAuxiliaryTablesVisible() ? 370f : 24f)
            : panelOffset.x;
        float py = Screen.height - panelOffset.y - ph;
        Rect panelRect = PanelLayout.Apply("PMPanel", new Rect(px, py, pw, ph));
        px = panelRect.x;
        py = panelRect.y;
        currentPanelRect = panelRect;
...
private bool AreAuxiliaryTablesVisible()
    {
    ...
    private void DrawDemandLabel(float dx, float dy, string combo, float p, float m, bool prominent)
    {
        ...
        string cText = cRatio >= UnityData.OutOfCurveRatio ? "fuera de curva" : cRatio.ToString("0.###");
    ...
    {
        return UnityData.CapacityRatio(currentCurve, demand.x, demand.y);
    }
