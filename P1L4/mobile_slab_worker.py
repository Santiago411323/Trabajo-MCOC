...
class Solver:
    ...
    def unit(self,node):
        ...
        disp={n['id']:list(ops.nodeDisp(n['id'])) for n in self.data['nodes']}
        ops.reactions()
        rz=sum(ops.nodeReaction(n['id'],3) for n in self.data['nodes'])
    ...
    def solve(self,request):
        ...
        for node,p in nodal.items():
            ...
            for k in disp: disp[k]=[a+p*b for a,b in zip(disp[k],u[k])]
